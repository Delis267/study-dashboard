...
STUDIENSTART = date(2025, 1, 1)
ZIELDATUM = date(2028, 1, 1)
STICHTAG_NACH_SECHS_MONATEN = date(2025, 7, 1)
TAG_VOR_STUDIENSTART = date(2024, 12, 31)
PROGNOSTIZIERTES_ENDE_BEI_AKTUELLER_VELOCITY = date(2025, 9, 30)
...
class ModulTest(unittest.TestCase):
    ...
    def test_modul_ohne_pruefungsleistung_ist_annerkannt(self) -> None:
        modul = self._anerkanntes_mathe_modul()
        self.assertIsNone(modul.pruefungsleistung)
        self.assertEqual(ModulStatus.ANERKANNT, modul.status)

    def _regulaeres_modul(
        self,
        kurs_id: str = "OOP",
    ...
    def _anerkanntes_mathe_modul(self) -> Modul:
...
class StudiumTest(unittest.TestCase):
    ...
    def test_modul_finden_liefert_modul_mit_passender_kurs_id(self) -> None:
        studium = self._studium_mit_oop_modul()
    ...
    def test_module_mit_gleicher_kurs_id_duerfen_nicht_doppelt_hinzugefuegt_werden(self) -> None:
        ...
        with self.assertRaises(ValueError):
            studium.modul_hinzufuegen(self._regulaeres_modul(kurs_id="OOP"))
    ...
    def test_notendurchschnitt_wird_mit_ects_gewichtet(self) -> None:
        studium = self._test_studium()
        oop = self._bestandenes_modul(kurs_id="OOP", note=2.0)
        datenbanken = self._bestandenes_modul(kurs_id="DB", ects=10, note=3.0)
    ...
    def test_ziel_notendurchschnitt_ist_erreicht_wenn_schnitt_besser_oder_gleich_ziel_ist(self) -> None:
        studium = self._test_studium(ziel_notendurchschnitt=2.3)
        studium.modul_hinzufuegen(self._bestandenes_modul(kurs_id="OOP"))
    ...
    def test_ziel_notendurchschnitt_ist_nicht_erreicht_wenn_schnitt_schlechter_als_ziel_ist(self) -> None:
        studium = self._test_studium(ziel_notendurchschnitt=2.0)
        studium.modul_hinzufuegen(self._bestandenes_modul(kurs_id="OOP"))
    ...
    def test_ziel_velocity_berechnet_noetige_ects_pro_monat_bis_zieldatum(self) -> None:
        studium = self._test_studium(gesamt_ects=180)
    ...
    def test_velocity_ist_null_am_studienstart(self) -> None:
        ...
        self.assertEqual(0.0, studium.velocity(STUDIENSTART))

    def test_velocity_nutzt_nur_fertige_eigenleistung_ects(self) -> None:
        studium = self._studium_mit_fertigem_anerkanntem_und_offenem_modul()

        self.assertEqual(1.68, studium.velocity(STICHTAG_NACH_SECHS_MONATEN))

    def test_velocity_darf_nicht_vor_studienstart_berechnet_werden(self) -> None:
        studium = self._test_studium()

        with self.assertRaises(ValueError):
            studium.velocity(TAG_VOR_STUDIENSTART)

    def test_prognose_ist_none_wenn_keine_velocity_vorliegt(self) -> None:
        studium = self._test_studium()

        self.assertIsNone(studium.prognostiziertes_ende(STICHTAG_NACH_SECHS_MONATEN))
    ...
    def test_prognose_berechnet_abschlussdatum_aus_offenen_ects_und_velocity(self) -> None:
        ...
        self.assertEqual(
            PROGNOSTIZIERTES_ENDE_BEI_AKTUELLER_VELOCITY,
            studium.prognostiziertes_ende(STICHTAG_NACH_SECHS_MONATEN),
        )

    def _test_studium(
        self,
        studiengang: str = "Software Engineering",
        startdatum: date = STUDIENSTART,
        zieldatum: date = ZIELDATUM,
        gesamt_ects: int = 180,
    ...
    def _studium_mit_oop_modul(self) -> Studium:
        studium = self._test_studium()
        studium.modul_hinzufuegen(self._regulaeres_modul(kurs_id="OOP"))
        return studium

    def _studium_mit_fertigem_anerkanntem_und_offenem_modul(
        self,
        gesamt_ects: int = 20,
    ) -> Studium:
        studium = self._test_studium(gesamt_ects=gesamt_ects)
        studium.modul_hinzufuegen(self._bestandene_oop_eigenleistung())
        studium.modul_hinzufuegen(self._anerkanntes_modul(kurs_id="MATHE"))
        studium.modul_hinzufuegen(self._regulaeres_modul(kurs_id="DB"))
        return studium
    ...
    def _bestandenes_modul(
        self,
        kurs_id: str,
        ects: int = 5,
        note: float = 2.3,
    ) -> Modul:
        modul = self._regulaeres_modul(kurs_id=kurs_id, ects=ects)
        modul.note_eintragen(note)
        return modul

    def _bestandene_oop_eigenleistung(self) -> Modul:
        return self._bestandenes_modul(kurs_id="OOP", ects=10)

    def _anerkanntes_modul(self, kurs_id: str, ects: int = 5) -> Modul:
        return Modul.anerkannt(
            kurs_id=kurs_id,
